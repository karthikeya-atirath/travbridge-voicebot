...
function AgentChatInput({ chatOpen, onSend = async () => {}, className }: AgentChatInputProps) {
  ...
  return (
    <div
      className={cn(
        'flex grow items-end gap-2 rounded-2xl bg-neutral-100 py-1 pr-1 pl-3 text-sm dark:bg-zinc-800',
        className
      )}
    >
      <textarea
        ...
        className="field-sizing-content max-h-24 min-h-10 flex-1 resize-none bg-transparent py-3 text-neutral-900 [scrollbar-width:thin] placeholder:text-neutral-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:text-neutral-100"
      />
      <Button
        size="icon"
        type="button"
...
export function AgentControlBar({
  ...
  return (
    <div
      ...
      className={cn(
        'flex w-full items-end gap-3 rounded-[32px] border border-neutral-200 bg-white p-2 shadow-lg dark:border-zinc-800 dark:bg-zinc-900',
        className
      )}
      {...props}
    >
      ...
      <div className="mb-[4px] flex shrink-0 items-center justify-end gap-2 pr-1">
        {/* Toggle Microphone */}
        ...
        {/* Disconnect - Desktop Only inline */}
      ...
      {/* Disconnect - Mobile Only Absolute Top Right */}
      {visibleControls.leave && (
        <div className="fixed top-4 right-4 z-[100] md:hidden">
          <AgentDisconnectButton
            ...
            className={cn(
              variant === 'livekit' &&
                'bg-destructive/10 dark:bg-destructive/10 text-destructive hover:bg-destructive/20 dark:hover:bg-destructive/20 focus:bg-destructive/20 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/4 rounded-full px-4 py-2 font-mono text-xs font-bold tracking-wider shadow-md backdrop-blur-md'
            )}
          >
            END
          </AgentDisconnectButton>
