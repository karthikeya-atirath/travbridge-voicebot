...
export function AudioVisualizer({
  ...
  switch (audioVisualizerType) {
    ...
    default: {
      ...
      let sizedClassName = cn('size-full', className);

      if (audioVisualizerBarCount <= 5) {
        size = 'xl';
        sizedClassName = cn('size-full gap-2', className);
      } else if (audioVisualizerBarCount <= 10) {
        size = 'lg';
        sizedClassName = cn('size-full gap-1', className);
      }
      ...
      return (
        <MotionAgentAudioVisualizerBar
          ...
          {...props}
        >
          <span className="min-h-1.5 w-1.5 rounded-full bg-current/20 transition-colors duration-250 ease-linear data-[lk-highlighted=true]:bg-current" />
        </MotionAgentAudioVisualizerBar>
      );
    }
  }
